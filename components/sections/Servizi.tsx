        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {servizi.map((servizio, index) => (
            <FadeIn key={servizio.id} delay={index * 0.1}>
              <Link href={`/servizi#${servizio.id}`} className="block h-full">
                <SpotlightCard className={`border border-border transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-accent/30 hover:-translate-y-1.5 hover:shadow-[0_12px_40px_rgba(30,48,80,0.08)] active:border-accent/30 active:shadow-sm h-full cursor-pointer ${index % 2 === 0 ? "bg-white" : "bg-white md:bg-white max-md:bg-surface"}`}>
